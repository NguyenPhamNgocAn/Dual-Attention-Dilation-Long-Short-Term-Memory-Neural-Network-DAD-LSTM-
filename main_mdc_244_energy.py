    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 1 == 0:# 10->1
            ...
            y_train_pred = predict(net, train_data,
                                   ...
                                   mode ="TRAIN")
            if( test_error_mae[-1] <0.3 and test_error_rmse[-1] < 0.7 and test_error_mape[-1] < 1.0):
                break
            if(e_i > n_epochs):
                break
            plt.figure()
...
# egg: egg.csv
# energy: energy.csv
raw_data = pd.read_csv(os.path.join("data","energy.csv"), nrows=100 if debug else None)
...
da_rnn_kwargs = {"batch_size": 128, "T": 10,"decoder_hidden_size": 64, "encoder_hidden_size": 64, "enc_size": 27}
config, model = da_rnn(data, n_targs=len(targ_cols),train_size=16000, valid_size =0, learning_rate=.001,hidden_structs_enc= hidden_structs_enc,
...
iter_loss, epoch_loss = train(model, data, config, n_epochs=71, save_plots=False)
