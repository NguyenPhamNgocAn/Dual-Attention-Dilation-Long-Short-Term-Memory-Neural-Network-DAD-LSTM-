...
def da_rnn(train_data: TrainData, n_targs: int, train_size:int, valid_size:int, hidden_structs_enc, dilations_enc, cells_enc,num_layer_enc, mode_enc,
    ...
    with open(os.path.join("data", "2drnn_enc_kwargs.json"), "w") as fi:
    ...
    with open(os.path.join("data", "2drnn_dec_kwargs.json"), "w") as fi:
...
plt.legend(loc='upper left')
plt.savefig('2rlstm_test_truth_origin_sml.png')
plt.close()
...
plt.savefig('2rlstm_loss_function.png')
...
plt.legend(loc='upper left')
plt.savefig('2rlstm_final_scaled_sml.png')
#plt.show()
#plt.pause(2)
plt.close()
#utils.save_or_show_plot("final_predicted.png", save_plots)
# saving information to file : da_rnn_kwargs.json
with open(os.path.join("data", "2rlstm_da_rnn_kwargs.json"), "w") as fi:
    json.dump(da_rnn_kwargs, fi, indent=4)

joblib.dump(scaler, os.path.join("data", "2rlstm_scaler.pkl"))
# saving values of hidden states of encoder + decoder
torch.save(best_model.encoder.state_dict(), os.path.join("data", "2rlstm_encoder.torch"))
torch.save(best_model.decoder.state_dict(), os.path.join("data", "2rlstm_decoder.torch"))
end = time.time()
'''print('model_encoder: ')
...
np.save('2rlstm_error_mae.npy',error_mae)
np.save('2rlstm_error_rmse.npy',torch.sqrt(error_mse))
np.save('2rlstm_error_mape.npy',error_mape)
np.save('2rlstm_time_darnn.npy', end- start)
np.save('2rlstm_train_predict.npy', pred_inverted[:,-1])
