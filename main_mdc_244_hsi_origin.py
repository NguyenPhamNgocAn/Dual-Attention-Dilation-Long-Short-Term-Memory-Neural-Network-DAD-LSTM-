...
# egg: egg.csv
raw_data = pd.read_csv(os.path.join("data","HSI.csv"), nrows=100 if debug else None)
...
config, model = da_rnn(data, n_targs=len(targ_cols),train_size=4277, valid_size =0, learning_rate=.001,hidden_structs_enc= hidden_structs_enc,
...
plt.title("Prediction on HSI data set")
plt.legend(loc='upper left')
plt.savefig('2drnn_test_truth_origin_hsi.png')
plt.close()
...
np.save('2drnn_predict_test_hsi.npy', pred_inverted[:,0])
