...
# egg: egg.csv
raw_data = pd.read_csv(os.path.join("data","DJIA.csv"), nrows=100 if debug else None)
...
iter_loss, epoch_loss,best_model = train(model, data, config, n_epochs=1, save_plots=False)
...
plt.figure()
plt.plot(pred_inverted[:,0],"o--", label = 'predict values')
plt.plot(train_inverted,"o--", label = 'real')
plt.title("Prediction on DJIA data set")
plt.legend(loc='upper left')
plt.savefig('none_test_truth_origin_djia.png')
plt.close()
...
np.save('none_predict_test_djia.npy', pred_inverted[:,0])
