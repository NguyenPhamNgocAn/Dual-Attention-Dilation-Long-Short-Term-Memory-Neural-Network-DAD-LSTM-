...
import copy
import utils
from modules_drnn import Encoder, Decoder
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            plt.savefig('result_drnn_vn_per_epoch_' + str(e_i)+'.png')
            #plt.show()
            #plt.pause(2)
            plt.close()
...
raw_data = pd.read_csv(os.path.join("data", "stockVN61.csv"), nrows=100 if debug else None)
...
iter_loss, epoch_loss,best_model = train(model, data, config, n_epochs=1000, save_plots=False)
...
plt.savefig('loss_function_drnn_vn.png')
...
print(error)
np.save('error_drnn_vn.npy', error)
np.save('time_drnn_vn.npy', end- start)
