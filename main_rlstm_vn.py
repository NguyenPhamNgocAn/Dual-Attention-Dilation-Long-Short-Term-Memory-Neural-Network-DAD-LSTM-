...
import copy
import utils
from modules_rlstm import Encoder, Decoder
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            plt.savefig('result_per_epoch_rlstm_vn_' + str(e_i)+'.png')
            #plt.show()
            #plt.pause(2)
            plt.close()
...
plt.savefig('loss_function_rlstm_vn.png')
plt.close()
...
plt.legend(loc='upper left')
plt.savefig('final_rlstm_vn.png')
#plt.show()
#plt.pause(2)
plt.close()
...
print(err)
np.save('error_rlstm_vn.npy', err)
np.save('time_rlstm_vn.npy', end - start)
