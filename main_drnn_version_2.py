...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            plt.savefig('result_drnn_version_2_per_epoch_' + str(e_i)+'.png')
            #plt.show()
            #plt.pause(2)
            #plt.close()
...
plt.savefig('loss_function_drnn_version_2.png')
...
plt.legend(loc='upper left')
plt.savefig('final_drnn_version_2.png')
plt.show()
#plt.pause(2)
plt.close()
...
print(error)
np.save('error_drnn_v_2.npy', error)
np.save('time_drnn_v_2.npy', end - start)
