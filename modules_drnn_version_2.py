...
from DRNN_PYTORCH.model_drnn import DilatedRNN
...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_structs,dilations,cells, encoder_hidden_size: int, decoder_hidden_size: int, T: int,out_feats=1):# them tham so: hidden_structs, dilations, cells
        ...
        self.drnn = DilatedRNN(T, decoder_hidden_size,1,hidden_structs,dilations,cells, num_layer= 4).to(device)
        #self.rlstm = Pred(100,100,1,1)
    def forward(self, input_encoded, y_history):
        ...
        hidden_states = torch.zeros((self.T, input_encoded.shape[0],self.decoder_hidden_size), device = device)
        for t in range(self.T - 1):
        ...
        # hidden_states : (self.T, batch_size, decoder_hidden_size)
        ...
        print('shape: ' )
        print(hidden_states.shape)'''
        drnn_output = self.drnn(hidden_states) # check shape (batch_size, 1)
        ...
        # Eqn. 22: final output
        #return self.fc_final(torch.cat((hidden[0], context), dim=1)) # (batch_size, 1)
