...
def da_rnn(train_data: TrainData, n_targs: int,encoder_hidden_size=64, decoder_hidden_size=64, T=10, learning_rate=0.01, batch_size=32):
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            plt.savefig('result_per_epoch_vn_' + str(e_i)+'.png')
            #plt.show()
            #plt.pause(2)
            plt.close()
...
plt.savefig('loss_function_vn.png')
...
print(error)
np.save('error_darnn_vn.npy',error)
np.save('time_darnn_vn.npy', end- start)
