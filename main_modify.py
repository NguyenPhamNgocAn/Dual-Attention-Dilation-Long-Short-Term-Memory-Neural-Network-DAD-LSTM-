...
import copy
import utils_1
from modules_1 import Encoder, Decoder
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    valid_error = np.zeros(int(n_epochs/10)) # computing valid error each 10 epoches
...
plt.savefig('loss_function_cuda0.png')
...
plt.legend(loc='upper left')
plt.savefig('final_cuda0.png')
#plt.show()
#plt.pause(2)
plt.close()
#utils.save_or_show_plot("final_predicted.png", save_plots)
# saving information to file : da_rnn_kwargs.json
with open(os.path.join("data/cuda_0", "da_rnn_kwargs_1.json"), "w") as fi:
    json.dump(da_rnn_kwargs, fi, indent=4)

joblib.dump(scaler, os.path.join("data/cuda_0", "scaler_1.pkl"))
# saving values of hidden states of encoder + decoder
torch.save(best_model.encoder.state_dict(), os.path.join("data/cuda_0", "encoder_1.torch"))
torch.save(best_model.decoder.state_dict(), os.path.join("data/cuda_0", "decoder_1.torch"))
...
print(end - start)
np.save('error_darnn_1.npy',error)
np.save('time_darnn_1.npy', end- start)
