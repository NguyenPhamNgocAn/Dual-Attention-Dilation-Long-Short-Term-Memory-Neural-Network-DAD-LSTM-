...
import copy
import utils
from  modules_drnn_version_2  import Encoder, Decoder
...
def da_rnn(train_data: TrainData, n_targs: int,hidden_structs,dilations,cells,encoder_hidden_size=64, decoder_hidden_size=64, T=10, learning_rate=0.01, batch_size=128):
    ...
    decoder = Decoder(**dec_kwargs, hidden_structs=hidden_structs, dilations = dilations,cells=cells).cuda(device)  # specify arguments for class decoder #////////////////////////////////////////////// trnsfer from "to" into cuda 
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            plt.savefig('result_drnn_version_2_vn_per_epoch_' + str(e_i)+'.png')
            #plt.show()
            #plt.pause(2)
            plt.close()
...
raw_data = pd.read_csv(os.path.join("data", "data_edit_pttt_arima.csv"), nrows=100 if debug else None)
...
input_dim = 64 # input the number of dimension of decoder_hidden_size in Decoder 
cell_type = "RNN"
assert (cell_type in ["RNN","LSTM","GRU"])
hidden_structs = [20]*4
dilations = [1,2,4,8]
assert (len(hidden_structs) == len(dilations))
cells = _contruct_cells(input_dim, hidden_structs, cell_type)
num_layer = 4
...
da_rnn_kwargs = {"batch_size": 16, "T": 10}
config, model = da_rnn(data, n_targs=len(targ_cols), learning_rate=.001,cells=cells ,hidden_structs = hidden_structs, dilations = dilations, **da_rnn_kwargs)
...
plt.savefig('loss_function_drnn_version_2_vn.png')
plt.close()
...
plt.legend(loc='upper left')
plt.savefig('final_drnn_version_2_vn.png')
#plt.show()
#plt.pause(2)
plt.close()
...
print('error: ')
error = final_y_pred - data.targs[config.train_size:]
error =np.mean(np.abs(error))
print(error)
np.save('error_drnn_v2_vn.npy', error)
np.save('time_drnn_v2_vn.npy', end - start)
