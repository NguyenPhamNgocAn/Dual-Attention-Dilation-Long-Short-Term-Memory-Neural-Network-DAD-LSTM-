...
# HSI.csv: train_size: 4277,enc_size: 5

da_rnn_kwargs = {"batch_size": 128, "T": 10,"decoder_hidden_size": 64, "encoder_hidden_size": 64, "enc_size": 81}
config, model = da_rnn(data, n_targs=len(targ_cols),train_size=32000, valid_size =0, learning_rate=.001,hidden_structs_enc= hidden_structs_enc,
...
plt.figure()
plt.plot(pred_inverted[:,-1], label = 'predict values')
plt.plot(train_inverted, label = 'real')
plt.title("Prediction on NASDAQ100(conditioned) data set")
plt.legend(loc='upper left')
plt.savefig('none_test_truth_origin_nasdaq_condition.png')
plt.close()
...
np.save('none_predict_test_nasdaq_condition.npy', pred_inverted[:,-1])
