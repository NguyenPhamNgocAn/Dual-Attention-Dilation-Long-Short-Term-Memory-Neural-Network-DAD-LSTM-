...
# egg: egg.csv
raw_data = pd.read_csv(os.path.join("data","SP.csv"), nrows=100 if debug else None)
...
da_rnn_kwargs = {"batch_size": 128, "T": 10,"decoder_hidden_size": 64, "encoder_hidden_size": 64, "enc_size": 5}
config, model = da_rnn(data, n_targs=len(targ_cols),train_size=4873, valid_size =0, learning_rate=.001,hidden_structs_enc= hidden_structs_enc,
...
iter_loss, epoch_loss = train(model, data, config, n_epochs=101, save_plots=False)
final_y_pred = predict(model, data, config.train_size,config.valid_size, config.batch_size, config.T, mode = "TEST")


pred_inverted = scaler.inverse_transform(np.repeat(final_y_pred,6,axis = 1))
...
plt.figure()
plt.plot(pred_inverted[:,0], label = 'predict values')
plt.plot(train_inverted, label = 'real')
plt.title("Prediction on S&P500 data set")
plt.legend(loc='upper left')
plt.savefig('none_test_truth_origin_sp.png')
plt.close()
...
torch.save(model.encoder.state_dict(), os.path.join("data/cuda_0", "encoder_1.torch"))
torch.save(model.decoder.state_dict(), os.path.join("data/cuda_0", "decoder_1.torch"))
...
np.save('none_predict_test_sp.npy', pred_inverted[:,0])
