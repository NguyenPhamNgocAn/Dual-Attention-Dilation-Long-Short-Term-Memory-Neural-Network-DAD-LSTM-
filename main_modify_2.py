...
import copy
import utils_2
from modules_2 import Encoder, Decoder
from custom_types import DaRnnNet, TrainData, TrainConfig
from utils_2 import numpy_to_tvar
from constants_2 import device
import time

start = time.time()
logger = utils_2.setup_log()
...
def da_rnn(train_data: TrainData, n_targs: int, train_size:int, valid_size:int, encoder_hidden_size=64, decoder_hidden_size=64, T=10, learning_rate=0.01, batch_size=128):
    ...
    encoder = Encoder(**enc_kwargs).cuda(device) # specify arguments for class encoder #///////////////////////////////////////////////
    # json.dump : saving information of a dictionary to a json file
    with open(os.path.join("data/cuda_1", "enc_kwargs_2.json"), "w") as fi:
        json.dump(enc_kwargs, fi, indent=4)

    dec_kwargs = {"encoder_hidden_size": encoder_hidden_size,
            "decoder_hidden_size": decoder_hidden_size, "T": T, "out_feats": n_targs}
    decoder = Decoder(**dec_kwargs).cuda(device)  # specify arguments for class decoder #////////////////////////////////////////////// trnsfer from "to" into cuda 
    with open(os.path.join("data/cuda_1", "dec_kwargs_2.json"), "w") as fi:
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=True):
    ...
    valid_error = np.zeros(int(n_epochs/10))
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            plt.savefig('result_per_epoch_cuda1_' + str(e_i)+'.png')
            #plt.show()
            #plt.pause(2)
            #plt.close()
    ...
    for y_i in range(0, len(y_pred), batch_size):
        ...
        for b_i, b_idx in enumerate(batch_idx):
            if mode =="TRAIN":
                ...
                idx = range(b_idx + train_size +valid_size- T, b_idx + train_size +valid_size - 1) 
...
config, model = da_rnn(data, n_targs=len(targ_cols),train_size=35100, valid_size=2730, learning_rate=.001, **da_rnn_kwargs)
iter_loss, valid_error, epoch_loss, best_model= train(model, data, config, n_epochs=100, save_plots=False)
...
plt.savefig('loss_function_cuda1.png')
plt.close()

plt.figure()
plt.semilogy(range(len(valid_error)), valid_error)
plt.title(" valid values ( each 10 epochs ) of validation set " )
plt.savefig('valid_cuda1.png')
plt.close()

plt.figure()
plt.plot(final_y_pred, label='Predicted')
print("final y predict size : " , final_y_pred.shape)
plt.plot(data.targs[config.train_size+config.valid_size:], label="True")
plt.title("y true vs y predict ")
print("y true size : " , data.targs[config.train_size+config.valid_size:].shape )
plt.legend(loc='upper left')
plt.savefig('final_cuda1.png')
#plt.show()
#plt.pause(2)
plt.close()
#utils.save_or_show_plot("final_predicted.png", save_plots)
# saving information to file : da_rnn_kwargs.json
with open(os.path.join("data/cuda_1", "da_rnn_kwargs_2.json"), "w") as fi:
    json.dump(da_rnn_kwargs, fi, indent=4)

joblib.dump(scaler, os.path.join("data/cuda_1", "scaler_2.pkl"))
# saving values of hidden states of encoder + decoder
torch.save(best_model.encoder.state_dict(), os.path.join("data/cuda_1", "encoder_2.torch"))
torch.save(best_model.decoder.state_dict(), os.path.join("data/cuda_1", "decoder_2.torch"))
...
print('error: ')
error = MAPELoss(numpy_to_tvar(final_y_pred), numpy_to_tvar(data.targs[config.train_size+config.valid_size:])).cpu()
print(error)
print("time complexity:")
print(end-start)
np.save('error_darnn_2.npy',error)
np.save('time_darnn_2.npy', end- start)
