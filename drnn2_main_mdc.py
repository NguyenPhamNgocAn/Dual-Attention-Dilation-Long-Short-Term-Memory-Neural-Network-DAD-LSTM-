...
def da_rnn(train_data: TrainData, n_targs: int, train_size:int, valid_size:int, hidden_structs_enc, dilations_enc, cells_enc,num_layer_enc, mode_enc,
    ...
    with open(os.path.join("data", "drnn2_enc_kwargs.json"), "w") as fi:
    ...
    with open(os.path.join("data", "drnn2_dec_kwargs.json"), "w") as fi:
...
hidden_structs_enc = [64]*2 # 64->128
dilations_enc = [2,4]
...
hidden_structs_dec = [128]*2 # 64->128
dilations_dec = [2,4]
...
# NASDAQ:  batch size: 128, enc size: 81 , train size:35100, valid size:2730,hidden_size = 64 both -> 128
...
plt.legend(loc='upper left')
plt.savefig('rlstm2_test_truth_origin_sml.png')
plt.close()
...
plt.savefig('rlstm2_loss_function.png')
...
plt.legend(loc='upper left')
plt.savefig('rlstm2_final_scaled_sml.png')
#plt.show()
#plt.pause(2)
plt.close()
#utils.save_or_show_plot("final_predicted.png", save_plots)
# saving information to file : da_rnn_kwargs.json
with open(os.path.join("data", "rlstm2_da_rnn_kwargs.json"), "w") as fi:
    json.dump(da_rnn_kwargs, fi, indent=4)

joblib.dump(scaler, os.path.join("data", "rlstm2_scaler.pkl"))
# saving values of hidden states of encoder + decoder
torch.save(best_model.encoder.state_dict(), os.path.join("data", "rlstm2_encoder.torch"))
torch.save(best_model.decoder.state_dict(), os.path.join("data", "rlstm2_decoder.torch"))
...
np.save('rlstm2_error_mae.npy',error_mae)
np.save('rlstm2_error_rmse.npy', torch.sqrt(error_mse))
np.save('rlstm2_error_mape.npy',error_mape)
np.save('rlstm2_time_darnn.npy', end- start)
np.save('rlstm2_train_predict_sml.npy', pred_inverted[:,-1])
