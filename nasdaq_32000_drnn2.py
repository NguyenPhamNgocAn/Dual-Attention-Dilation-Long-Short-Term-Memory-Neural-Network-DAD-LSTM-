...
plt.legend(loc='upper left')
plt.savefig('rlstm2_test_truth_origin_' + name +'.png')
plt.close()
...
plt.savefig('rlstm2_loss_function_' + name +'.png')
...
plt.legend(loc='upper left')
plt.savefig('rlstm2_final_scaled_' + name + '.png')
#plt.show()
#plt.pause(2)
plt.close()
#utils.save_or_show_plot("final_predicted.png", save_plots)
# saving information to file : da_rnn_kwargs.json
with open(os.path.join("data/cuda_0", "rlstm2_da_rnn_kwargs.json"), "w") as fi:
    json.dump(da_rnn_kwargs, fi, indent=4)

joblib.dump(scaler, os.path.join("data/cuda_0", "rlstm2_scaler.pkl"))
# saving values of hidden states of encoder + decoder
torch.save(best_model.encoder.state_dict(), os.path.join("data/cuda_0", "rlstm2_encoder.torch"))
torch.save(best_model.decoder.state_dict(), os.path.join("data/cuda_0", "rlstm2_decoder.torch"))
...
print(end - start)
np.save('error_rlstm2_mae_' + name + '.npy',error)
np.save('error_rlstm2_mape_' + name + '.npy',error_mape)
np.save('error_rlstm2_rmse_' + name + '.npy',error_rmse)
np.save('rlstm2_time_' + name + '.npy', end- start)
np.save('rlstm2_predict_test_' + name + '.npy', pred_inverted[:,0])
