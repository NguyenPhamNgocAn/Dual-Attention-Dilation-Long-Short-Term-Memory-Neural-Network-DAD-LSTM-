...
def da_rnn(train_data: TrainData, n_targs: int, train_size:int, valid_size:int, hidden_structs_enc, dilations_enc, cells_enc,num_layer_enc, mode_enc,
    ...
    with open(os.path.join("data", "drnn1_enc_kwargs.json"), "w") as fi:
    ...
    with open(os.path.join("data", "drnn1_dec_kwargs.json"), "w") as fi:
...
plt.legend(loc='upper left')
plt.savefig('rlstm1_test_truth_origin_sml.png')
plt.close()
...
plt.savefig('rlstm1_loss_function.png')
...
plt.legend(loc='upper left')
plt.savefig('rlstm1_final_scaled_sml.png')
#plt.show()
#plt.pause(2)
plt.close()
#utils.save_or_show_plot("final_predicted.png", save_plots)
# saving information to file : da_rnn_kwargs.json
with open(os.path.join("data", "rlstm1_da_rnn_kwargs.json"), "w") as fi:
    json.dump(da_rnn_kwargs, fi, indent=4)

joblib.dump(scaler, os.path.join("data", "rlstm1_scaler.pkl"))
# saving values of hidden states of encoder + decoder
torch.save(best_model.encoder.state_dict(), os.path.join("data", "rlstm1_encoder.torch"))
torch.save(best_model.decoder.state_dict(), os.path.join("data", "rlstm1_decoder.torch"))
...
np.save('rlstm1_mae_darnn.npy',error_mae)
np.save('rlstm1_rmse_darnn.npy',torch.sqrt(error_mse))
np.save('rlstm1_mape_darnn.npy',error_mape)
np.save('rlstm1_time_darnn.npy', end- start)
np.save('rlstm1_train_predict_sml.npy', pred_inverted[:,-1])
